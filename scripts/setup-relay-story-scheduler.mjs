/**
 * 連続小説「今日のお題」— Cloud Scheduler（毎日 0:00 JST）
 *
 * 前提: `.env.rakuda-api.local` に ROBO_PICKUP_CRON_SECRET
 * 用法: node scripts/setup-relay-story-scheduler.mjs
 */
...
const JOB_ID = 'relay-story-daily';
const URI = 'https://rakuda.coffee/api/relay-story/cron-daily';
...
function loadCronSecret() {
  ...
  if (!fs.existsSync(file)) {
    console.error('[setup-relay-story-scheduler] .env.rakuda-api.local がありません');
    process.exit(1);
  }
  ...
  console.error('[setup-relay-story-scheduler] ROBO_PICKUP_CRON_SECRET が未設定です');
  process.exit(1);
}
...
function runGcloud(args) {
  if (process.platform === 'win32') {
    if (!fs.existsSync(GCLOUD)) {
      console.error('[setup-relay-story-scheduler] gcloud not found:', GCLOUD);
      process.exit(1);
    }
...
if (!exists) {
  baseArgs.push('--description=RakudaRelayStoryDailyPrompt');
}

console.log(`[setup-relay-story-scheduler] ${exists ? 'update' : 'create'} ${JOB_ID} (${LOCATION})`);
runGcloud(baseArgs);
...
console.log('[setup-relay-story-scheduler] OK — 毎日 0:00 JST に POST', URI);
